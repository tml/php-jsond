...
static PHP_MINIT_FUNCTION(jsond)
{
	...
	PHP_JSOND_REGISTER_LONG_CONSTANT("DUPKEYS_AS_ERROR", PHP_JSON_DUPKEYS_AS_ERROR);
	...
	PHP_JSOND_REGISTER_LONG_CONSTANT("ERROR_DUPKEY", PHP_JSON_ERROR_DUPKEY);
...
#define PHP_JSON_ERROR_DUPKEY_MSG "Duplicate key found when decoding"
...
static PHP_JSOND_FUNCTION(last_error_msg)
{
	...
	switch(JSOND_G(error_code)) {
		...
		case PHP_JSON_ERROR_DUPKEY:
			RETURN_STRINGL(PHP_JSON_ERROR_DUPKEY_MSG, sizeof(PHP_JSON_ERROR_DUPKEY_MSG) - 1, 1);
		default:
